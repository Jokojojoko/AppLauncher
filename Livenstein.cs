...
namespace AppLauncher
{
    class Livenstein
    {
        ...
        public static int Distance(string firstText, string secondText)
        {
        ...
        public static string MinName(Dictionary<string, int> dict, bool Forsed = false)
        {
            string min_name = "";
            int min_num = Int32.MaxValue;
            foreach (var item in dict)
            {
                if (item.Value < min_num)
                {
                    min_num = item.Value;
                    min_name = item.Key;
                }
